...
//
// v0.2.0 - OWA Ink Calculator
// 2025-01-XX

// Added
// - Screen printing ink consumption calculations
// - Automatic test print (+1 shirt) and 1.2% buffer
// - Per shirt and total consumption display
// - Auto-detection of selected object color for text labels
// - Simplified interface focused on screen printing needs
...
function initExportInfo(exportInfo,jsonData) {
    ...
    dataExportInfo = ["inch",3,true,false,true,true,"",false,0,"1:10",0,"m",1,false,false,"",0,"",0, false, 1, "White"];
    dataJson = ["unitSingle","unitSingleIndex","singleUnit","doubleUnit","allShapes","textLabel","textLabelColor","useRealArea","realAreaScale","realAreaValue","realAreaUnitScale","realAreaUnit","decimals","countShapes","unitDoubleOne","unitDoubleOneIndex","unitDoubleTwo","unitDoubleTwoIndex","intersectingPaths","shirtCount","inkName"]
...
function settingDialog(exportInfo) {
    ...
    var dlgGetShapeArea = new Window("dialog"); 
        dlgGetShapeArea.text = "OWA Ink Calculator"; 
    ...
    // INK NAME GROUP
    // ==============
    var grpInkName = pnlSettings.add("group", undefined, {name: "grpInkName"}); 
        grpInkName.orientation = "row"; 
        grpInkName.alignChildren = ["left","center"]; 
        grpInkName.spacing = 10; 
        grpInkName.margins = 0;

    var txtInkName = grpInkName.add("statictext", undefined, undefined, {name: "txtInkName"}); 
        txtInkName.text = "Ink Name"; 

    var txtInkNameInput = grpInkName.add("edittext", undefined, undefined, {name: "txtInkNameInput"}); 
        txtInkNameInput.text = "White"; 
        txtInkNameInput.preferredSize.width = 120;

    // SHIRT COUNT GROUP
    // =================
    var grpShirtCount = pnlSettings.add("group", undefined, {name: "grpShirtCount"}); 
        grpShirtCount.orientation = "row"; 
        grpShirtCount.alignChildren = ["left","center"]; 
        grpShirtCount.spacing = 10; 
        grpShirtCount.margins = 0;

    var txtShirtCount = grpShirtCount.add("statictext", undefined, undefined, {name: "txtShirtCount"}); 
        txtShirtCount.text = "Number of shirts"; 

    var txtShirtCountInput = grpShirtCount.add("edittext", undefined, undefined, {name: "txtShirtCountInput"}); 
        txtShirtCountInput.text = "1"; 
        txtShirtCountInput.preferredSize.width = 50; 
    ...
    var addTextLabelCB = cmykBlackGrp.add("checkbox", undefined, undefined, {name: "addTextLabelCB"}); 
        addTextLabelCB.helpTip = "Add text label with ink consumption data."; 
    ...
    // All other controls removed for simplified interface 
    ...
    // Double units functionality removed


    // Simplified interface - no complex controls needed
    ...
    exportInfo.unitSingle = "inch"; // Fixed to inches
    exportInfo.unitSingleIndex = 3; // inch index in the array
    exportInfo.singleUnit = true; // Always single unit (inches)
    exportInfo.doubleUnit = false;
    exportInfo.shirtCount = parseInt(txtShirtCountInput.text) || 1;
    exportInfo.inkName = txtInkNameInput.text || "White";
    exportInfo.textLabel = addTextLabelCB.value;
    
    // Set default values for removed controls
    exportInfo.useRealArea = false; 
    exportInfo.realAreaScale = 0;
    exportInfo.realAreaValue = "1:10";
    exportInfo.realAreaUnitScale = 0; 
    exportInfo.realAreaUnit = "m"; 
    exportInfo.decimals = 3; // Fixed to 3 decimals
    exportInfo.allShapes = false; // Don't show individual shapes
    exportInfo.countShapes = false; // Don't count shapes
    exportInfo.unitDoubleOne = "inch"; // Fixed to inches
    exportInfo.unitDoubleOneIndex = 3; // inch index
    exportInfo.unitDoubleTwo = "cm"; // Secondary unit for display
    exportInfo.unitDoubleTwoIndex = 1; // cm index
    exportInfo.intersectingPaths = false; // Default to false
    
    // Color detection will be done when placing text labels
...
function saveToJSON(exportInfo){
    ...
    unitDoubleTwo = exportInfo.unitDoubleTwo;
    unitDoubleTwoIndex = exportInfo.unitDoubleTwoIndex;
    shirtCount = exportInfo.shirtCount;
    inkName = exportInfo.inkName;
    var saveData = {
...
function getShapeArea(exportInfo, dup, obj, pos){
    ...
    if (totalArea!=0){
        ...
        // Add ink name at the top
        display.unshift(exportInfo.inkName + " Ink:");
        
        if(!exportInfo.singleUnit){
            // display.push("Total Area: " + conv[unit[1]].toFixed(decimalPlaces) + unit[1]+"² / " + conv[unit[2]].toFixed(decimalPlaces) + unit[2]+"²");
            if (exportInfo.useRealArea) display.push("Real Area: "+conv['rw'].toFixed(decimalPlaces) +exportInfo.realAreaUnit+"²")
            // if (exportInfo.useRealArea) display.unshift("Real Area: "+conv['rw'].toFixed(decimalPlaces) +"m²")
            display.push("Total Area: " + conv[exportInfo.unitDoubleOne].toFixed(decimalPlaces) + exportInfo.unitDoubleOne+"² / " + conv[exportInfo.unitDoubleTwo].toFixed(decimalPlaces) + exportInfo.unitDoubleTwo+"²");
        } else {
            if (exportInfo.useRealArea) display.push("Real Area: "+conv['rw'].toFixed(decimalPlaces) +exportInfo.realAreaUnit+"²")
            // if (exportInfo.useRealArea) display.unshift("Real Area: "+conv['rw'].toFixed(decimalPlaces) +"m²")
            display.push("Total Area: " + conv[exportInfo.unitSingle].toFixed(decimalPlaces) + exportInfo.unitSingle+"²");
        }
        
        // Calculate ink consumption for screen printing
        var areaInInches = conv.inch;
        var inkPerSquareInch = 0.065; // grams per square inch
        var totalShirts = exportInfo.shirtCount + 1; // Add test print (+1 shirt)
        var baseInkConsumption = areaInInches * inkPerSquareInch * totalShirts;
        var totalInkConsumption = baseInkConsumption * 1.012; // Add 1.2% buffer
        var perShirtConsumption = areaInInches * inkPerSquareInch;
        
        display.push("Per shirt: " + perShirtConsumption.toFixed(decimalPlaces) + " grams");
        display.push("Total (" + totalShirts + " shirts + 1.2% buffer): " + totalInkConsumption.toFixed(decimalPlaces) + " grams");
...
function placeTextLabel(data, obj, pos){
    ...
    // Auto-detect color from selected object
    var textColor;
    if (app.activeDocument.selection.length > 0) {
        var selectedObj = app.activeDocument.selection[0];
        var objColor = checkItem(selectedObj);
        if (objColor.valid && objColor.fill) {
            textColor = objColor.fill;
        } else {
            // Default color if no fill found
            if (space=='CMYK'){
	...
	for(i=0;i<areaLabel.paragraphs.length;i++){
		...
		areaLabel.paragraphs[i].fillColor = textColor;
